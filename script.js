...
async function handleAuth() {
    ...
    try {
        if (isLoginMode) {
            ...
            const userCredential = await createUserWithEmailAndPassword(window.auth, userEmail, passwordInput);
...
function renderSidebar() {
    ...
    Object.keys(dataObj).forEach(key => {
        ...
        editBtn.innerHTML = "✏️";
        editBtn.onclick = (e) => { e.stopPropagation(); renameItem(key); };
...
function updateCardUI() {
    ...
    if (currentSection === "lugat") {
        ...
        document.getElementById('current-lesson-title').textContent = currentLesson || "Dars tanlanmagan";
        document.getElementById('card-counter').textContent = words.length > 0 ? `${currentIndex + 1} / ${words.length}` : "0 / 0";
...
function toggleMode() {
    isEnToUz = !isEnToUz;
    document.getElementById('mode-toggle-btn').textContent = isEnToUz ? "Rejim: EN ➔ UZ" : "Rejim: UZ ➔ EN";
    updateCardUI();
}
...
async function handleAddNewSidebarItem() {
    ...
    if (currentSection === "lugat") {
        if (userLessonsData[val]) { alert("Bu dars allaqachon mavjud!"); return; }
        userLessonsData[val] = []; currentLesson = val;
    } else {
        if (userSpeakingData[val]) { alert("Bu mavzu allaqachon mavjud!"); return; }
        userSpeakingData[val] = []; currentTopic = val;
    }
    input.value = "";
    await saveUserDataToDB();
    renderSidebar();
...
async function renameItem(oldKey) {
    ...
    if (currentSection === "lugat") {
        userLessonsData[newKey] = userLessonsData[oldKey]; delete userLessonsData[oldKey];
        if (currentLesson === oldKey) currentLesson = newKey;
    } else {
        userSpeakingData[newKey] = userSpeakingData[oldKey]; delete userSpeakingData[oldKey];
...
async function handleAddWord() {
    ...
    if (!currentLesson) { alert("Iltimos, oldin dars yarating!"); return; }
    if (!enInp.value.trim() || !uzInp.value.trim()) { alert("Maydonlarni to'ldiring!"); return; }

    userLessonsData[currentLesson].push({ en: enInp.value.trim(), uz: uzInp.value.trim() });
    enInp.value = ""; uzInp.value = "";
...
async function handleAddSpeaking() {
    ...
    if (!currentTopic) { alert("Iltimos, mavzu tanlang!"); return; }
    if (!qInp.value.trim() || !aInp.value.trim()) { alert("Savol va javobni to'ldiring!"); return; }

    userSpeakingData[currentTopic].push({ question: qInp.value.trim(), answer: aInp.value.trim() });
    qInp.value = ""; aInp.value = "";
    await saveUserDataToDB();
    updateCardUI();
}
...
function openWordsModal() {
    const words = userLessonsData[currentLesson] || [];
    if (words.length === 0) { alert("Bu darsda hali so'zlar mavjud emas!"); return; }
...
async function clearCurrentLessonWords() {
    ...
    if (confirm(`"${currentLesson}" darsidagi barcha so'zlarni tozalashni xohlaysizmi?`)) {
        userLessonsData[currentLesson] = []; currentIndex = 0;
        await saveUserDataToDB();
        updateCardUI();
    }
}
...
function initDOMEvents() {
    ...
    if (toggleToRegister) {
        toggleToRegister.onclick = (e) => {
            ...
            authSwitchText.innerHTML = 'Akkauntingiz bormi? <a href="#" id="toggle-to-login">Tizimga kirish</a>';
            document.getElementById('toggle-to-login').onclick = (el) => {
                el.preventDefault(); isLoginMode = true;
                authTitle.textContent = "Tizimga Kirish"; authSubmitBtn.textContent = "Kirish";
                authSwitchText.innerHTML = 'Akkauntingiz yo\'qmi? <a href="#" id="toggle-to-register">Ro\'yxatdan o\'tish</a>';
                initDOMEvents();
            };
        };
    }
